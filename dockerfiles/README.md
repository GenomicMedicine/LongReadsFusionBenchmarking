...
This directory contains Docker containers for fusion tools and the LongFuse k235 integration pipeline, providing reproducible analysis environments.
...
| LongFuse k235 Pipeline | `longfuse-k235-pipeline_docker/` | Integration | ✓ |

## Quick Start

### Downloading (Pulling) Pre-built Containers to Local Server (Recommended)

To save time and ensure a consistent environment, we highly recommend downloading the pre-built Docker containers directly to your local server from DockerHub instead of building them from scratch:

```bash
# Pull the LongFuse pipeline environment
docker pull mark614/long-read_rna-seq_fusion_detection_benchmark:longfuse_pipeline

# Pull individual caller environments
docker pull mark614/long-read_rna-seq_fusion_detection_benchmark:ctat-lr-fusion
docker pull mark614/long-read_rna-seq_fusion_detection_benchmark:jaffal
docker pull mark614/long-read_rna-seq_fusion_detection_benchmark:longgf
docker pull mark614/long-read_rna-seq_fusion_detection_benchmark:fusionseeker
docker pull mark614/long-read_rna-seq_fusion_detection_benchmark:flair-fusion
docker pull mark614/long-read_rna-seq_fusion_detection_benchmark:pbfusion
docker pull mark614/long-read_rna-seq_fusion_detection_benchmark:ifdlong
docker pull mark614/long-read_rna-seq_fusion_detection_benchmark:genion
```

### Building All Containers from Scratch

If you must modify the Dockerfiles or prefer to build all Docker images locally on your server from scratch:
...
for tool_dir in */; do
    ...
    docker build -t "mark614/long-read_rna-seq_fusion_detection_benchmark:${tool_name}" .
    cd ..
done
```
...
All pre-built Docker images are officially hosted on Docker Hub under the `mark614` namespace:

```bash
# Example: Run CTAT-LR-Fusion directly via the pre-built image
docker run -v /data:/data mark614/long-read_rna-seq_fusion_detection_benchmark:ctat-lr-fusion \
