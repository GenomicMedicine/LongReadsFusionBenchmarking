...
{  
   ...
   echo "  -m, --minimap2_opt      How many cores are assigned to run the pipeline in parallel. Use single core by default"
...
$minimap2 $minimap2_opt $genomeDir $InFile | samtools view -Sb | samtools sort -o $BAMfile
